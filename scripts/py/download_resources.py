...
def get_huggingface_token():
    """
    Get the Hugging Face token from environment or ~/.netrc.

    Checks HF_TOKEN first, then ~/.netrc (machine huggingface.co).

    @return: The Hugging Face token if found, otherwise None.
    """
    ...
    try:
        ...
        auths = netrc.netrc(netrc_filepath).authenticators(NETRC_MACHINE)
        if auths:
            _, _, token = auths
            return token
        ...
        logging.warning("Failed to parse ~/.netrc: %s", e)
    except Exception as e:
        logging.warning("Unexpected error reading ~/.netrc: %s", e)

    logging.warning(
        "Hugging Face token not found in %s or ~/.netrc (machine %s).",
        HF_TOKEN_ENV,
        NETRC_MACHINE,
    )
    return None
...
def get_huggingface_hub_error_types():
    ...
    This stays lazy to avoid hard requiring `huggingface_hub` just to import this script.
    """
    try:
...
def download_file(url: str, dest: Path) -> None:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to create directory {dest.parent}") from e
    try:
        timeout_seconds = DEFAULT_DOWNLOAD_TIMEOUT_SECONDS
        try:
            timeout_seconds = int(os.environ.get(DOWNLOAD_TIMEOUT_ENV, str(DEFAULT_DOWNLOAD_TIMEOUT_SECONDS)))
        except Exception:
            logging.warning(
                "Invalid %s; defaulting to %s seconds.",
                DOWNLOAD_TIMEOUT_ENV,
                DEFAULT_DOWNLOAD_TIMEOUT_SECONDS,
            )
        with urllib.request.urlopen(url, timeout=timeout_seconds) as g:
...
def validate_download(filepath, expected_hash):
    ...
    """
    expected = normalize_sha256_hex_digest(expected_hash)
    if not is_sha256_hex_digest(expected):
        raise ValueError(f"Invalid sha256sum format: {expected_hash}")
